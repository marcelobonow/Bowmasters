...
public class CameraBehaviour : MonoBehaviour
{
    ...
    [SerializeField]
    private Transform startPointPlayer;//Posição do personagem do player
    [SerializeField]
    private Transform startPointEnemy;//Posição do personagem do inimigo
    [SerializeField]
    private float moveTimer; //Timer usado para fazer o lerp
    ...
    void SmoothFollow(Vector2 position)
    {
        ...
        transform.position = new Vector3(Mathf.Lerp(transform.position.x, position.x, moveTimer / 50),
                Mathf.Lerp(transform.position.y, position.y + 1, moveTimer / 50), ZCAMERA);//offset no eixo y da camera
